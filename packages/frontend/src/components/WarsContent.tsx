...
import { ExpandedRowContent } from "@/components/WarsContent/ExpandedRowContent";
import { Card, CardContent } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { Button } from "@/components/ui/button";
import { AlertCircle, RefreshCw, Swords } from "lucide-react";

interface WarsContentProps {
  initialRanking: PlayerStats[];
  organization: any;
  organizations: any[];
  clan?: any;
}
...
export function WarsContent({ initialRanking, organization, organizations, clan }: WarsContentProps) {
  const [ranking, setRanking] = useState<PlayerStats[]>(initialRanking);
  ...
  const loadRanking = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getWarRanking(clanTag.replace("#", ""), selectedMonths, true);
      setRanking(data);
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Ranking de Guerras</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Desempenho dos jogadores nas guerras do clã
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <RankingList
          ...
          title="Ranking de Guerras Random"
          selectedMonthsLabel={monthsLabel(selectedMonths)}
          renderExpanded={(player) => <ExpandedRowContent player={player as PlayerStats} />}
        />
      )}
    </div>
  );
